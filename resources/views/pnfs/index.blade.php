        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    ...
                    <a href="{{ route('pnfs.create') }}" class="btn btn-primary"><i class="bi bi-plus-lg"></i> Crear Nuevo</a>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @foreach ($Pnfs as $Pnf)
                                @if ($Pnf->estatus) <!-- Mostrar solo registros activos por defecto -->
                                    <tr class="registro-activo">
                                        <td>{{ $Pnf->nombre }}</td>
                                        <td>{{ $Pnf->descripcion }}</td>
                                        <td>{{ $Pnf->estatus ? 'Activo' : 'Inactivo' }}</td>
                                        <td>
                                            <a href="{{ route('pnfs.show', $Pnf->id) }}" class="btn btn-success"><i class="bi bi-eye"></i> Ver</a>
                                            <a href="{{ route('pnfs.edit', $Pnf->id) }}" class="btn btn-warning"><i class="bi bi-pencil"></i> Editar</a>
                                            <form action="{{ route('pnfs.destroy', $Pnf->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @else <!-- Registros inactivos (ocultos por defecto) -->
                                    <tr class="registro-inactivo" style="display: none;">
                                        ...
                                        <td>{{ $Pnf->estatus ? 'Activo' : 'Inactivo' }}</td>
                                        <td>
                                            <form action="{{ route('pnfs.activate', $Pnf->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('PUT')
